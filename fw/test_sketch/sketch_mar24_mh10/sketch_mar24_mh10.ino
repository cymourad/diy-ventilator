...
void loop() { // continuous loop
  ...
  while (HM10.available() > 0) { // if HM10 sends smth, read
  ...
  // for later on, delete all the code below and write the code to control the machine
  // must first break down this string/int to get the diff parts
  ...
  switch (inData.charAt(0) {
    case "B": // Bi-PAP mode
      ...
      // TODO set the PIP (inpiration pressure)
      delay(
    }
}
